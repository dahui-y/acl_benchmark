...
------------------------------------------------------------------------
⚠️ 跑完之后的定位变更（2026-08-10）：**这把核验器降级为附录里的敏感度
分析，不做主指标。** 三条实测理由：

  ① 它没解决当初要解决的那个例子。03/2025 基线里船 (2159x688) 和它正下方
     的倒影 (1214x501) 核验分是 +0.0799 / +0.0403，**都判"保留"** ——
     负例里专门写了 "a reflection on water" 也没用。
     诊断：零假设太容易。115 个已知假阳性来自"在冰川上查 house"这类
     离谱错误，而真正麻烦的是**近似正确**的错（倒影像船、浪花像人）。
     用离谱的错定阈值，必然放过近似的错。
  ② 它删掉了真东西。03/1234 我们这版核验后计数 0，excess -1。
  ③ 改善幅度随零假设的定义漂：不核验 -67.9%，旧零假设(n=4) -69.4%，
     新零假设(n=115) -76.8%。**幅度不稳定，所以它不能当主指标。**

主指标改用已发表协议（CountGen CVPR'25 / GenEval NeurIPS'23），
见 cross_detector.py —— 那套协议里根本没有这一步，检测器 + 类名匹配 +
数个数而已。我们自造的核验步骤正是审稿人会攻击的自由度。
------------------------------------------------------------------------
"""
