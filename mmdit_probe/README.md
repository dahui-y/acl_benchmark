...
## 第一轮的结果与结论（2026-08-06）

**第一轮的设计是错的，结论作废。** 记录在这里，因为错法本身是有信息量的。

跑出来：三个交换条件（img / txt / both）**全部输出风格图的复制品**——
`C = 0.140 / 0.162`（内容布局全没）、`L = 0.999 / 0.925`（输出结构 = 风格图结构）、
`S ≈ 0.002`（输出**就是**风格图）。两路都饱和，所以两路之间的比较不携带任何信息。

两个叠加的错：

1. **共享初始噪声。** 本意是"让差异可归因"，实际效果相反：两个分支的 image stream
   在 block 0 是**同一个张量**，此时把 K/V 全换过去，数学上必然收敛到风格分支。
   `swap both` 精确复现风格图，**这是理论预测的唯一输出，不是测量结果**。
   内容分支从头就没有自己的内容，`C` 这个量在该设计下无意义。
2. **全 block、全时间步注入。** 强度旋钮拧到底。所有已录用方法都只注入子集
   （StyleID 用 decoder 后段层且**跳过早期时间步**，因为早期步定布局）。
   第一轮从没访问过"部分注入"这个区间，而那是唯一可能出现"迁风格但留内容"的区间。

**这一轮确认为真的两件事：**

- **管道正确。** `swap both` 精确复现风格图（S=0.0018, L=1.000）是数学上唯一正确的
  输出；加上自检的 `style 0 / content 255`，注入机制完全按预期工作。
- **存在第三条通路。** `swap txt` 的方差是 `swap img` 的 **50 倍**（±0.139 vs ±0.003），
  肉眼也看得出残差（海浪浪花、茶壶角度、自行车画法、狮子鬃毛、城堡塔楼都不同）。
  来源是 **SD3 的 pooled CLIP embedding 走 AdaLN 调制通路，不经过任何 K/V 投影**，
  hook 碰不到它。任何关于"MMDiT 里风格住在哪"的主张都必须把这条通路算进去。

第二轮见 `sweep.py`：独立噪声 + 24 格强度扫描（block 三分段 × 时间步三档 × 两路）。

---

## 第二轮结果：**方向死了**（2026-08-06）

24 格全跑完（150 张图，SD3.5-medium，seed 0）。**没有任何一格做到了风格迁移。**

### 关键量：`corr(1-C, dS) = 0.874`

24 格全部落在**同一条权衡线**上——从"什么都没发生"（dS 0, C 1）到
"风格图的逐像素复制"（dS 0.467, C 0.128）。**风格是拿内容一比一换来的。**

风格迁移这件事的定义就是**脱离这条线**：高 C 的同时拿到高 dS。所以判据不能用
绝对阈值（第一版报告就是这么错的：`dS>0.05 且 C>0.5` 会把线上的点判成 YES），
**要用相对这条线的残差**。

| 最大残差 | |
|---|---|
| image 流 | **+0.074**（`img__late__skip50`） |
| text 流 | **−0.001**（`txt__late__skip50`） |

**没有一格脱线。** 而且看图就知道那个 +0.074 是什么：`img__late__skip50` 是
**满屏椒盐噪点**，`img__early__skip50` 是**马赛克方块**。纹理 Gram 把这些高频退化
读成了"风格更接近"，其实是画坏了。唯一像样的一点是**调色板迁移**
（椅子那行染上茶壶的粉底、灯塔那行退成铅笔纸的米色）——**颜色过来了，纹理没有。**

### 三条确认的事实

1. **MMDiT 上 K/V 替换没有"迁移区间"。** 只有"没效果"和"覆盖"两端，中间是退化。
   这和 U-Net 定性不同：StyleID 在 decoder self-attention 上做同样的替换，
   能拿到一个明显脱线的点。**U-Net 那套配方在 MMDiT 上没有对应形式。**
2. **text 流在早期步之后完全惰性。** 所有 `txt__*__skip25/skip50` 都是空操作
   （dS ≈ 0，C 0.88–1.00，`txt__late__skip50` 的 C = **0.998**）。
   MMDiT 的文本条件在前 ~7 步被吃完，之后不再起作用。
3. **存在第三条通路。** 第一轮发现的 pooled CLIP → AdaLN 调制，不经过任何 K/V 投影。

### 结论

**这不是"移植"，是"连移植都不成立"。** 按事先约定，方向到此为止。
