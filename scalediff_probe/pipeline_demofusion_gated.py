...
# ============================ GATED FORK ============================
# fork 自 help_code/DemoFusion/pipeline_demofusion_sdxl.py（逐字节拷贝后
# 只加钩子）。门 = self.gate（外部赋 DemoGate 实例；None = 不装 = 原版）。
# 三个钩：phase-1 结束 finalize / patch 支 set_patch_views /
# dilated 支 set_dilated_views。除此之外一行未改。
# =====================================================================
...
class DemoFusionSDXLPipeline(DiffusionPipeline, FromSingleFileMixin, LoraLoaderMixin, TextualInversionLoaderMixin):
    ...
    ###################################################### Phase Initialization ########################################################
        ...
        # -- GATED FORK 钩 1：phase-1 录制结束，定格门控图 --
        _gate = getattr(self, "gate", None)
            ...
            with self.progress_bar(total=num_inference_steps) as progress_bar:
                for i, t in enumerate(timesteps):
                    ...
                    for j, batch_view in enumerate(views_batch):
                        ...
                        # -- GATED FORK 钩 2：告知门当前批的窗坐标（含 jitter 偏移）--
                        if _gate is not None:
                            _gate.set_patch_views(
                                batch_view, jitter_range,
                    ...
                    for j, batch_view in enumerate(views_batch):
                        # -- GATED FORK 钩 3：告知门当前批的跨步子网格 --
                        if _gate is not None:
                            _gate.set_dilated_views(
                                batch_view, current_scale_num, h_pad, w_pad,
                                latents.shape[-2], latents.shape[-1])
                        latents_for_view = torch.cat(
                            [
