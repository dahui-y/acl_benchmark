...
1. 六族现象里，**reciprocal 与 part-whole 两族的真值是否真的单值**。
   ...
   可能存在合法变体——**先做一轮 pilot，凡真值不唯一的族直接砍掉**，
   宁可只留 3 族也不留一族说不清的。
2. 检测器在**同类小物体密集**场景的实测准确率（气球、苹果、杯子），
   GenEval 的 κ=0.823 是它们自己的 prompt 分布，不是我们的。**必须自测。**
3. 现有 T2I 模型对 "each" / "together" 是否**完全无反应**。
   如果 `dist` 与 `coll` 的数量分布完全相同，那是干净的系统性失败（可发表）；
   但要先在 pilot 上确认这不是因为**两句话都退化成同一张图**
   （那就变成"模型忽略副词"的平凡结论）。
4. 再扫一轮别名：`plural predication`、`cumulative reading`、
   `scope ambiguity in image generation`、`numeral quantifier T2I`。
   目前 9 轮检索没打出对应物，但这**不等于已核实为空**。
