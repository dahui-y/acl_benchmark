# This file may have been modified by modified from https://github.com/huggingface/diffusers/blob/main/src/diffusers/pipelines/controlnet/pipeline_controlnet_inpaint_sd_xl.py
...
import src.utils.images_utils as ptp_utils
...
class StableDiffusionXLControlNetInpaintPipeline(
    ...
    def encode_prompt(
        ...
        if do_classifier_free_guidance and negative_prompt_embeds is None and zero_out_negative_prompt:
            ...
            for i, (negative_prompt, tokenizer, text_encoder) in enumerate(zip(uncond_tokens, tokenizers, text_encoders)):
                if isinstance(self, TextualInversionLoaderMixin):
                ...
                # 这里negative_prompt_2作为negative prompt inversion使用，因此要只对后面一半插值
                if npi_interp > 0.0 and i > 0:
                    zero_input = tokenizer(
                        [""],
                        padding="max_length",
                        max_length=max_length,
                        truncation=True,
                    ...
                    zero_prompt_embeds = text_encoder(
                        zero_input.input_ids.to(device), 
                        output_hidden_states=True,
                        )
                    zero_pooled_prompt_embeds = zero_prompt_embeds[0]
                    zero_prompt_embeds = zero_prompt_embeds.hidden_states[-2]
                    negative_pooled_prompt_embeds = ptp_utils.slerp_tensor(npi_interp, negative_pooled_prompt_embeds, zero_pooled_prompt_embeds)
                    negative_prompt_embeds = ptp_utils.slerp_tensor(npi_interp, negative_prompt_embeds, zero_prompt_embeds)
    ...
    def prepare_latents(
        ...
        if latents is None and add_noise:
            ...
            '''
            tmp_dtype = torch.float16
            prev_sample = latents.to(dtype=tmp_dtype)
            self.vae.to(dtype=tmp_dtype)
            prev_sample = prev_sample.to(dtype=tmp_dtype)
            image = self.vae.decode(prev_sample, return_dict=False)[0]
            ...
            noise_tmp = randn_tensor(shape, generator=generator, device=device, dtype=dtype) # inv_guidance
            latents_tmp = self.scheduler.add_noise(image_latents, noise_tmp, timestep)
            # latents = latents.to(device) * self.scheduler.init_noise_sigma #
            latents = latents.to(device) * self.scheduler.init_noise_sigma + 0.2 * latents_tmp
        else:
            noise = randn_tensor(shape, generator=generator, device=device, dtype=dtype)
    ...
    def __call__(
        ...
        style_embeddings_instruct: Optional[torch.Tensor] = None,
        style_guidance_scale: float = 0,
        content_embeddings_instruct: Optional[torch.Tensor] = None,
        content_guidance_scale: float = 0,
        ip_instruct_model: Optional[IPAdapterInstructSDXL] = None,
        CSD_model: Optional[IPAdapterInstructSDXL] = None,# Optional[CSD_CLIP] = None,
        inv_guidance: float = 0.95,
        ...
        **kwargs,
        ...
        # 3.2 Get style image prompt into prompt_embeds
        ...
        if self.do_classifier_free_guidance:
            if do_NPT:
                prompt_embeds = torch.cat([content_embedding, style_embedding, prompt_embeds], dim=1)
                negative_prompt_embeds = torch.cat([neg_style_embedding, neg_content_embedding, negative_prompt_embeds], dim=1)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if style_guidance_scale > 0.0 or content_guidance_scale > 0.0:
                    guidance, latents, best_style_sim, best_content_sim = self.cond_fn(
                        latents,
                        t,
                        i,
                        prompt_embeds,
                        ...
                        added_cond_kwargs,
                        noise_pred,
                        style_embeddings_instruct,
                        style_guidance_scale,
                        best_style_sim,
                        content_embeddings_instruct,
                        content_guidance_scale,
                        best_content_sim,
                        ip_instruct_model,
                        CSD_model,
                    ...
                    noise_pred -= guidance * self.rescale_guidance(guidance,noise_pred_text,noise_pred_uncond,guidance_scale)
