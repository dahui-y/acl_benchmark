...
def sanity(clip, A, d):
    ...
    if abs(bic) < 1.0:
        print("  -> **CLIP score 在 4096² 上与质量无关。** 高斯模糊那一行实测"
              "**+1.18%**（糊了反而涨）\n     —— 半径 8 px 缩到 320² 只剩 0.6 px，"
              "根本没进视觉塔。所以 ±1% 是噪声。\n     我们的 +0.02% 只能读作"
              "'语义没变'，细节这件事 CLIP 给不了任何信息。")
        print("\n  **不要把这条扩大解释成'领域的指标看不见细节'。**"
              "AccDiffusion v2 自己就把 CLIP\n  定位成 'an additional *alignment* "
              "metric'，而他们测细节的是 FID_c / IS_c（每张图裁 10 个\n  原分辨率"
              " patch 再算，那是看得见细节的）。领域只对**物体级计数**盲，"
              "缺口就这一条。\n  细节请跑 detail_check.py（频谱）或 FID_c。")
    else:
